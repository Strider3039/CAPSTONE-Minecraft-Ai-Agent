...
def main() -> int:
    ...
    inst = Path(__file__).resolve().parent
    if str(inst) not in sys.path:
        sys.path.insert(0, str(inst))
    from bridge_install_artifacts import write_bridge_data_artifacts

    paths = write_bridge_data_artifacts(bridge_dir)
    overlay_path = paths["overlay_path"]
    drop_in = paths["minecraft_drop_in"]
    if paths.get("created_overlay"):
    ...
    print(f"[Installer] Minecraft config drop-in (copy to instance config/): {drop_in}")
    ...
    print(f"[Installer] Point the Forge mod at this Data folder: copy\n  {drop_in}\n  to <Minecraft instance>/config/ai_agent_bridge_data_path.txt")
